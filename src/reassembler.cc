...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  if ( first_index >= first_unacceptable_index_
       || first_index + data.size() <= next_assembled_index_ ) { // data 不在接收窗口的范围内
    return;
  }
  ...
  uint64_t count
    = std::min( first_unacceptable_index_ - start_index, data.size() - offset ); // Done: data.size() - offset
  string to_storge_data = data.substr( offset, count );

  uint64_t to_storge_data_end = start_index + to_storge_data.size();

  auto last_iter
    = buffer_.lower_bound( to_storge_data_end ); // 找到一个块，这个块的首索引大于等于要插入的数据的尾部
